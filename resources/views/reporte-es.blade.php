    ...
    @foreach ( $serviceVisit->serviceReports as $idx => $report )
        ...
        <table>
            <tr>
                <td style="width:70%;text-align: left;">
                    <p style="font-weight: normal;font-size:15px;">REPORTE DE SERVICIO @if($loop->count>1) ({{ $loop->iteration }}/{{ $loop->count }}) @endif</p>
        ...
        <table class="tblStyle">
            <tr>
                <td style="width:40%;">
                    ...
                    {{ $serviceVisit->branch->client->name }}
                </td>
                <td style="width:60%;">
                    DIRECCIÓN:<br>
                    {{ $serviceVisit->branch->address }}
                </td>
            </tr>
        </table>
        <table class="tblStyle">
            <tr>
                <td style="width:35%;">
                    CONTACTO:<br>
                    {{ $serviceVisit->branchManager->name }}
                </td>
                <td>
            ...
            <tr>
                ...
                @foreach ($catalogCodes as $code)
                    <td>
                        ...
                        {{ $code->code }}
                        @if ($code->id === $codeId)
                        <p style="margin: 0;padding:0;border:0;font-size:8px">{{ __('codes.'.$code->id,[],'es', $code->description) }}</p>
                        @endif
                    </td>
                @endforeach
            </tr>
        ...
        <div style="position: absolute; bottom: 0;width:100%;">
            <div class="defaultBorder" style="float: left;width: 48%;height:100px;">
                REALIZADO POR:
                <div style="width:100%;text-align:center;margin-top:70px;">{{ $serviceVisit->user->nombre }} {{ $serviceVisit->user->apellido_paterno }} {{ $serviceVisit->user->apellido_materno }}</div>
            </div>
