    ...
    <div>
        <div class="panel border-[#e0e6ed] dark:border-[#1b2e4b]">
            ...
            <form class="space-y-5" @submit.prevent="submit">
                ...
                <div>
                    <label for="formActive">{{ $t("errors.edit.active") }}</label>
...
const form = reactive({
    name: (props.error.name ?? '').toUpperCase(),
    es: (props.error.es ?? '').toUpperCase(),
    pt: (props.error.pt ?? '').toUpperCase(),
...
function submit() {
    ...
    router.put(`/reports/errors/${props.error.id}`, form, {
        onSuccess: () => {
            console.log("Exitoso");
            Swal.close();
            Swal.fire({
